...
var LaunchDungeons=function()
{
	...
	new Game.Monster('Sentient Furnace','sentientFurnace',[0,3],0,{hp:60,might:14,guard:12,speed:4,dodge:0,rarity:1},bossLoot);
	...
	new Game.Monster('Ascended Baking Pod','ascendedBakingPod',[1,3],0,{hp:60,might:12,guard:14,speed:4,dodge:0,rarity:0.7},bossLoot);
	...
	Game.Entity=function(type,subtype,dungeon,pic,stats)
	{
		...
		this.GoTo=function(x,y)
		{
			...
			{
				this.targets=[];
				if (this.x==x) this.targets.push([1,0],[-1,0]);
				if (this.y==y) this.targets.push([0,1],[0,-1]);
				this.Move();
			}
		}
		...
		this.Turn=function()
		{
			if (this.type=='monster')
			{
				...
				for (var i=0;i<howManyTurns;i++)
				{
					if (1==1)
					{
						if (this.AI=='flee') this.Flee(this.dungeon.heroEntity.x,this.dungeon.heroEntity.y);
						else
						{
							this.GoTo(this.dungeon.heroEntity.x,this.dungeon.heroEntity.y);
							if (this.stuck || this.targets.length==[]) this.Wander();
						}
					}
				}
			}
			...
			if (this.type=='hero')
			{
				...
				for (var i in entities)
				{
					if (entities[i].type=='item' && entities[i].subtype=='cookies')
					{
						...
						var value=Math.ceil(entity.value*Game.Objects[this.dungeon.type].amount*50*(1+Math.random()*((this.stats.luck)/20)));
						if (value>0)
						{
				...
				if (tile!=-1)
				{
					...
					if (Math.random()<0.2)
					{
						...
						for (var ii in Game.Monsters)
						{
							...
							if (me.level!=0 && me.level<=(depth+this.level) && Math.random()<(me.stats.rarity||1)) monsters.push(me.name);
						}
						if (monsters.length>0)
						{
							this.AddEntity('monster',choose(monsters),tile[0],tile[1]);
					...
					else
					{
						if (Math.random()<0.6)
						{
							...
							if (value>0)
							{
								var entity=this.AddEntity('item','cookies',tile[0],tile[1]);
								entity.value=value;
							}
						}
